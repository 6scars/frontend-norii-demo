...
import { APP_ROUTES, getArtistRoute } from '../../app/routes.ts'
...
import { buildNowPlayingModel } from '../../modules/Player/now-playing-model.ts'
...
export default function NowPlayingPage() {
  ...
  const model = useMemo(
    () => buildNowPlayingModel({ currentSong, playbackQueue, songs }),
    [currentSong, playbackQueue, songs],
  )
  ...
  return (
    <div className="now-playing-page">
      ...
      <header className="now-playing-topbar">
        <button className="now-playing-back" onClick={() => { void navigate(-1) }} type="button"><Icon name="chevronLeft" size={18} /> Powrót</button>
        <span>Teraz gra</span>
          ...
          {model.activeSong ? (
            <>
              <section aria-labelledby="now-playing-title" className="now-playing-artwork-section">
                ...
                <div className="now-playing-cover">{artwork ? <img alt={`Okładka utworu ${model.activeSong.song_name ?? 'Bez tytułu'}`} src={artwork} /> : <span>{(model.activeSong.song_name ?? 'U').slice(0, 1)}</span>}</div>
                <div className="now-playing-progress">
                  <input aria-label="Pozycja utworu" disabled={!duration} max={duration || 0} min="0" onChange={seek} step="0.1" style={{ '--range-fill': `${duration ? (currentTime / duration) * 100 : 0}%` } as CSSProperties} type="range" value={Math.min(currentTime, duration || 0)} />
                ...
                <div className="now-playing-controls">
                  ...
                  <button aria-label="Poprzedni utwór" onClick={() => { void goToPrevious() }} type="button"><Icon name="previous" size={26} /></button>
                  ...
                  <button aria-label="Następny utwór" onClick={() => { void goToNext() }} type="button"><Icon name="next" size={26} /></button>
        ...
        <aside aria-label="Kolejka odtwarzania" className="now-playing-queue">
          ...
          <div className="now-playing-queue__list">
            {model.queue.map((song, index) => {
              ...
              return <button aria-current={isActive ? 'true' : undefined} className={isActive ? 'now-playing-song now-playing-song--active' : 'now-playing-song'} key={`${getSongId(song)}-${index}`} onClick={() => { void selectSong(song) }} type="button"><span className="now-playing-song__index">{isActive ? <span className="now-playing-song__bars"><i /><i /><i /></span> : index + 1}</span><span className="now-playing-song__cover">{songArtwork ? <img alt="" src={songArtwork} /> : (song.song_name ?? 'U').slice(0, 1)}</span><span className="now-playing-song__copy"><strong>{song.song_name}</strong><small>{song.author}</small></span><Icon name="more" size={17} /></button>
            })}
          </div>
