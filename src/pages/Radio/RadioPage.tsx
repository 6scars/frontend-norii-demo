...
import { buildRadioModel } from '../../modules/Radio/radio-model.ts'
import type { RadioStationId } from '../../modules/Radio/radio-model.ts'
...
export default function RadioPage() {
  ...
  const [selectedStationId, setSelectedStationId] = useState<RadioStationId>('catalog')
  ...
  const playTrack = async (songId: EntityId | null) => {
    ...
    if (index < 0) return
    setPlaybackQueue(selectedStation.tracks)
    setCurrentTrackIndex(index)
    const selectedId = getSongId(selectedStation.tracks[index])
    if (selectedId !== null) await chooseSong(selectedId)
  }

  const startStation = () => {
    const firstSong = selectedStation?.tracks[0]
    if (firstSong) void playTrack(getSongId(firstSong))
  }
  ...
  return (
    <div className="radio-page">
      ...
      <section aria-labelledby="radio-queue" className="radio-queue">
        ...
        <TrackTable currentSongId={getSongId(currentSong)} emptyMessage="Ta stacja nie ma jeszcze utworów." onPlay={(id) => { void playTrack(id) }} tracks={selectedStation.tracks} />
      </section>
    </div>
  )
}
