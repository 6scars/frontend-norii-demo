...
import type { usePlaybackQueue } from '../../modules/Player/usePlaybackQueue.ts'
...
function QueueSong({ song, active = false, onPlay }: QueueSongProps) {
  return (
    <button className={`queue-song ${active ? 'queue-song--active' : ''}`} onClick={onPlay}
      ...
      <span className="queue-song__cover">{getArtworkUrl(song) ? <img alt="" src={getArtworkUrl(song) ?? undefined} loading="lazy" /> : null}</span>
...
interface QueuePanelProps {
  playback: ReturnType<typeof usePlaybackQueue>
  onClose: () => void
}

export default function QueuePanel({ playback, onClose }: QueuePanelProps) {
      ...
      <div className="queue-panel__current">
        ...
        {currentSong ? (
          <div className="queue-song queue-song--current">
            <div className="queue-song__cover">{getArtworkUrl(currentSong) ? <img alt="" src={getArtworkUrl(currentSong) ?? undefined} /> : null}</div>
